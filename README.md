...
> 아래 이미지는 추후 `images` 폴더에 업로드할 예시 화면을 기준으로 연결해두었습니다.

| 메인/외환 서비스 화면 | AI·OCR 기능 화면 |
| --- | --- |
| ![FLOBANK 예시 화면 1](images/ex1.png) | ![FLOBANK 예시 화면 2](images/ex2.png) |
...
```mermaid
flowchart LR
    User[사용자] --> App[Flutter Mobile App]
    App --> API[Spring Boot API Server]
    API --> Security[Spring Security / JWT]
    API --> OracleM[(Oracle Master DB)]
    OracleM --> OracleS[(Oracle Slave DB)]
    API --> Redis[(Redis Queue)]
    Redis --> Worker[Async Log Worker]
    API --> AI[OpenAI / Gemini / ML Kit]
    API --> GCP[GCP Deploy]
    GCP --> CICD[GitHub Actions CI/CD]
```
